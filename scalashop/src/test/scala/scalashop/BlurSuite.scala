...
class BlurSuite extends FunSuite {
  ...
  test("boxBlurKernel at the edge") {
    val src = new Img(3, 4)
    src(0, 0) = 0; src(1, 0) = 1; src(2, 0) = 2
    src(0, 1) = 3; src(1, 1) = 4; src(2, 1) = 5
    src(0, 2) = 6; src(1, 2) = 7; src(2, 2) = 8
    ...
    assert(boxBlurKernel(src, 2, 2, 1) === 8,
      s"(boxBlurKernel(2, 2, 1) should be 8, " +
        s"but it's ${boxBlurKernel(src, 2, 2, 1)})")
  }
  ...
  test("HorizontalBoxBlur.parBlur with radius 1 should correctly blur the first two rows of a 3x3 image") {
    val w = 3
    val h = 3
    val src = new Img(w, h)
    val dst = new Img(w, h)
