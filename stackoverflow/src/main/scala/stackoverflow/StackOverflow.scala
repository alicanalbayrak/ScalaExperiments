...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    val qidQuestions = postings.filter(question => question.postingType == 1).map(question => (question.id : QID, question: Question))
    val qidAnswer = postings.filter(answer => answer.postingType == 2 && answer.parentId.nonEmpty).map(answer => (answer.parentId.get : QID, answer: Answer))

    qidQuestions.join(qidAnswer).groupByKey()
  }
