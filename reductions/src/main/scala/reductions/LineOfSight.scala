...
object LineOfSight {
  ...
  def upsweep(input: Array[Float], from: Int, end: Int, threshold: Int): Tree = {
    if (end - from <= threshold) {
      val r = upsweepSequential(input, from, end)
      Leaf(from, end, r)
    } else {
      val mid = from + (end - from) / 2
      val (p1, p2) = parallel(upsweep(input, from, mid, threshold), upsweep(input, mid, end, threshold))
      Node(p1, p2)
    }
  }
